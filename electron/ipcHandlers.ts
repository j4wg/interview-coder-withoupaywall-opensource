...
export function initializeIpcHandlers(deps: IIpcHandlerDeps): void {
  ...
  ipcMain.handle("voice-start-listening", async () => {
    try {
      ...
      await deps.voiceHelper?.startListening();
      return { success: true };
    } catch (error: any) {
  ...
  ipcMain.handle("voice-stop-listening", async () => {
    try {
      await deps.voiceHelper?.stopListening();
      return { success: true };
    } catch (error: any) {
      log.error("Voice: Error stopping listening:", error);
      return { success: false, error: error.message };
    }
  });
  ...
  ipcMain.handle("voice-get-status", () => {
    try {
      return (
        deps.voiceHelper?.getStatus() || {
          isListening: false,
          hasOpenAI: false,
        }
      );
    } catch (error: any) {
      log.error("Voice: Error getting status:", error);
      return { isListening: false, hasOpenAI: false };
    }
  });
