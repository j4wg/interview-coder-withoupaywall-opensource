...
const electronAPI = {
  ...
  onVoiceListeningStarted: (callback: () => void) => {
    const subscription = () => callback();
    ipcRenderer.on("voice-listening-started", subscription);
    return () => {
      ipcRenderer.removeListener("voice-listening-started", subscription);
    };
  },
  onVoiceListeningStopped: (callback: () => void) => {
    const subscription = () => callback();
    ipcRenderer.on("voice-listening-stopped", subscription);
    return () => {
      ipcRenderer.removeListener("voice-listening-stopped", subscription);
    };
  },
  onVoiceQuestionDetected: (callback: (data: { text: string }) => void) => {
    const subscription = (_: any, data: { text: string }) => callback(data);
    ipcRenderer.on("voice-question-detected", subscription);
    return () => {
      ipcRenderer.removeListener("voice-question-detected", subscription);
    };
  },
  onVoiceListeningError: (callback: (error: string) => void) => {
    const subscription = (_: any, error: string) => callback(error);
    ipcRenderer.on("voice-listening-error", subscription);
    return () => {
      ipcRenderer.removeListener("voice-listening-error", subscription);
    };
  },
};
